...
def Denature(section): 
    
    dna = " ";
    
    if section.strand2 != " ":
            
        dna = DNA(section.strand2[:], False);
        section.strand2 = " ";
...
def Anneal(section, primers, bindingSites, availablePrimers, limitPrimers):
...
def RunExtension(section, ampSegLen, primerLen, lengthDistributions):
    ...
    # This will determin if a full copy is managed to be made, and the length of the partial if not. 
...
def PCR():
    ...
    dnaContainer.append(RunDenaturation(dnaContainer[0]));
    ...
    while completeCycles < cycleCount:
        ...
        for section in dnaContainer:
            ...
            detachedStrand = Denature(section); 
            
            if detachedStrand != " ":
               dnaContainer.append(detachedStrand);
            
            Anneal(section, primers, bindingSites, availablePrimers, limitPrimers); 
            retVal = RunExtension(section, ampSegLen, primerLen, lengthDistributions); 
